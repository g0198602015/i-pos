package com.jeromelibrary.XMLUtiltiy;
...
public class DOMDocument
{
    public static Document createDocument(String filepath, String decoder)
    {
        ...
        catch(Exception ex)
        {
            ...
            Log.getInstance().WriteLog(fullClassName, methodName, ex.toString() + "[" + _strXML + "]", Log.LogLevel.ERROR);
    ...
    public static Document createDocument(InputStream in)
    {
        ...
        catch(Exception ex)
        {
            ...
            String className = fullClassName.substring(fullClassName.lastIndexOf(".") + 1);
            String methodName = Thread.currentThread().getStackTrace()[2].getMethodName();
            Log.getInstance().WriteLog(fullClassName, methodName, ex.toString(), Log.LogLevel.ERROR);
            return null;
        }
    }
    ...
    private static String ConvertInputStreamToString(InputStream in, String charset)
    {
        ...
        catch(Exception ex)
        {
            ...
            String className = fullClassName.substring(fullClassName.lastIndexOf(".") + 1);
            String methodName = Thread.currentThread().getStackTrace()[2].getMethodName();
            Log.getInstance().WriteLog(fullClassName, methodName, ex.toString());
            return "";
        }
    }
}